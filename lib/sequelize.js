...
class Sequelize {
  ...
  constructor(database, username, password, options) {
    ...
    if (arguments.length === 1 && typeof database === 'object') {
      ...
      options = options || {};
      if (options.dialect === 'dbisam') {
        if (options.bindParam === null || options.bindParam === undefined) {
          options.bindParam = false;
        }
        config = { ...options, database, username, password };
      } else {
    ...
    switch (this.getDialect()) {
      ...
      case 'sqlite':
        ...
        break;
      case 'dbisam':
        Dialect = require('./dialects/dbisam');
        break;
      default:
