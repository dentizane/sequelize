...
class QueryGenerator {
  ...
  */
  addIndexQuery(tableName, attributes, options, rawTablename) {
    ...
    } else if (this._dialect.name === 'dbisam' && options.type === 'FULLTEXT') {
      ind = [
        'ALTER TABLE',
        tableName
      ];
    ...
    ind = ind.concat(
      ...
      options.type === 'TEXT' ? undefined : this.quoteIdentifiers(options.name),
      this._dialect.supports.index.using === 1 && options.using ? `USING ${options.using}` : '',
      !(this._dialect.supports.indexViaAlter || options.type === 'TEXT') ? `ON ${tableName}` : undefined,
