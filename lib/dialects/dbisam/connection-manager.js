...
class ConnectionManager extends AbstractConnectionManager {
  ...
  buildConnString(config) {
    ...
    if (config.DSN) {
      ...
      connStr += 'DRIVER={DBISAM 4 ODBC Driver};';
      
      if (config.connType === 'LOCAL') {
        connStr += `CatalogName=${config.database}`;
      } else if (config.connType === 'REMOTE') {
        if (config.host) {
          connStr += /([0-9]{3}\.){3}[0-9]{3}/.test(config.host) ? 'RemoteIPAddress=' : 'RemoteHostName=';
          connStr += `${config.host};`;
        }
        connStr += config.username ? `UID=${config.username};` : '';
