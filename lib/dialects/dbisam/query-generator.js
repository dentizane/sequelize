...
const { BLOB, NUMBER } = require('../../data-types');
...
class DBISAMQueryGenerator extends AbstractQueryGenerator {
  ...
  escape(value, field, options) {
    if (field && field.type instanceof BLOB) {
      return `Base64ToBLOB('${value}')`;
    }

    if (field && field.type && (field.type instanceof NUMBER || field.type.escape === false)) {
    ...
    return super.escape(value, field, options);
  }
  ...
  format(value, field, options, bindParam) {
    ...
    return super.format(value, field, options, bindParam);
  }
}
