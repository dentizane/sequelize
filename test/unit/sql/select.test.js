...
describe(Support.getTestDialectTeaser('SQL'), () => {
  describe('select', () => {
    ...
    testsql({
      ...
      dbisam: 'SELECT "email", "first_name" AS "firstName" FROM "User" WHERE "User"."email" = \'jon.snow@gmail.com\' ORDER BY "email" DESC TOP 10;'
    });
    ...
    it('include (subQuery alias)', () => {
      ...
      expectsql(sql.selectQuery('User', {
        ...
        mssql: 'SELECT [User].*, [postaliasname].[id] AS [postaliasname.id], [postaliasname].[title] AS [postaliasname.title] FROM ' +
          ...
          'INNER JOIN [Post] AS [postaliasname] ON [User].[id] = [postaliasname].[user_id];',
        dbisam: 'SELECT "User".*, "postaliasname"."id" AS "postaliasname.id", "postaliasname"."title" AS "postaliasname.title" FROM ' +
          '(SELECT "User"."name", "User"."age", "User"."id" AS "id" FROM "User" AS "User" ' +
          'WHERE ( SELECT "user_id" FROM "Post" AS "postaliasname" WHERE ("postaliasname"."user_id" = "User"."id") TOP 1 ) IS NOT NULL) AS "User" ' +
          'INNER JOIN "Post" AS "postaliasname" ON "User"."id" = "postaliasname"."user_id";'
      });
    });
